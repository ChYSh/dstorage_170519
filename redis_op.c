...
int rop_set_string(redisContext *conn,const char* key, char* value)
{
	...
	//rop_test_reply_type(reply);
	if (reply == NULL) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]Add or increment table: %s,value: %s Error:%s\n", key, value,conn->errstr);
...
int rop_get_string(redisContext *conn,const char* key, char* value)
{
	...
	//rop_test_reply_type(reply);
	if (reply == NULL) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]Add or increment table: %s,Error:%s\n", key,conn->errstr);
...
int rop_hget_hashes(redisContext *conn,const char* key, const char *field, char * value)
{
	int retn = 0;	
	...
	//rop_test_reply_type(reply);
	if (reply == NULL) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]Add or increment table: %s,%s, Error:%s\n", key, field,conn->errstr);
...
int rop_hset_hashes(redisContext *conn,const char* key, const char *field, const char* value)
{
	int retn = 0;	
	...
	//rop_test_reply_type(reply);
	if (reply == NULL) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]Add or increment table: %s,%s,value: %s Error:%s\n", key, field, value,conn->errstr);
